		...
		<div className="lg:flex lg:items-center lg:gap-20  xl:gap-[150px] lg:justify-start">
			<div className="lg:pl-[22px] lg:py-5">
				<div className="hidden lg:block lg:w-64">
					<CurrentSong
						singerName={songs[trackIndex]?.attributes?.singerName}
						songName={songs[trackIndex]?.attributes?.songName}
						imageSrc={songs[trackIndex]?.attributes?.image}
					/>
				</div>
			</div>
			...
			<div className="relative lg:static w-full lg:w-[450px] xl:w-[600px] 2xl:w-[837px]  lg:h-[75px]">
				{songs[trackIndex]?.attributes?.image ? (
					<img
						src={songs[trackIndex]?.attributes?.image}
						alt=""
				...
				<p className="absolute w-full text-xs font-semibold text-center text-theme-green-700 bottom-6 lg:hidden">
					{songs[trackIndex]?.attributes?.songName}
				</p>
				...
				<AudioPlayer
					...
					src={songs[trackIndex]?.attributes?.songLink}
